/**
 * Batch 1 browser verification via Chrome DevTools Protocol.
...
const USER_DATA =
  ...
  `${process.env.TEMP}\\echochat-batch1-chrome-${CDP_PORT}-${Date.now()}`;
  ...
  try {
    ...
    try {
      spawn(
        ...
        [
          "-Command",
          `Get-CimInstance Win32_Process -Filter "Name='chrome.exe'" | Where-Object { $_.CommandLine -like '*echochat-batch1*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }`,
        ],
        { stdio: "ignore" }
      );
    } catch {}
